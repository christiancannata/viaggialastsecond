<div class="row " data-update-route="work-experience" data-id="{{$work['id']}}">
    ...
    <div class="form hide">
        <div class="col s12 m12 l12">
            <div class="card  grey-meritocracy-user">
                <form class="edit-work-experience" data-id="{{$work['id']}}">

                    <input type="hidden" name="id_work_experience" value="{{$work['id']}}">
                    <div class="container">
                        <div class="row">
                            <div class="input-field col s12 m6">
                                <i class="prefix mdi-action-work active"></i>
                                <input type="text" value="{{$work['role']}}"
                                       name="role" id="role_{{$work['id']}}">
                                <label class="active"
                                       for="role_{{$work['id']}}">{{ trans('wizard-application.ruolo') }}</label>
                            </div>

                            <div class="input-field col s12 m6">
                                <select required id="job_function_{{$work['id']}}"
                                        name="job_function[id]">
                                    <option value="" disabled
                                    >{{ trans('profile.scegli_job') }}</option>
                                    @foreach($jobFunctions as $function)
                                        <option value="{{$function['id']}}"
                                                @if($function['id']==$work['job_function']['id']) selected @endif>{{$function['name']}}</option>
                                    @endforeach
                                </select>
                                <label for="job_function_{{$work['id']}}">{{ trans('wizard-application.job_function') }}</label>
                            </div>


                            <div class="input-field col s12 m6">
                                <i class="mdi-action-store prefix active"></i>
                                <input type="text" id="azienda_{{$work['id']}}"
                                       name="company_plain_text"
                                       value="{{$work['company_plain_text']}}">
                                <label class="active"
                                       for="azienda_{{$work['id']}}">{{ trans('wizard-application.nome_azienda') }}</label>
                            </div>


                            <div class="input-field col s12 m6">
                                <select name="industry[id]" id="industry_{{$work['id']}}">
                                    <option value="" disabled
                                ...
                                <label for="industry_{{$work['id']}}">{{ trans('wizard-application.industry') }}</label>
                            </div>
                            ...
                            <div class="input-field col s12 m6">
                                ...
                                <input type="text" required=""
                                       ...
                                       id="luogo_lavoro_{{$work['id']}}" autocomplete="off"
                                       aria-required="true">
                                <label class="active"
                                       for="luogo_lavoro_{{$work['id']}}">{{ trans('wizard-application.luogo_lavoro') }}</label>
                            ...
                            <div class="input-field col s12 m6">
                                ...
                                <label class="active"
                                       for="data_inizio_{{$work['id']}}">{{ trans('wizard-application.data_inizio') }}</label>
                                <input aria-required="true" type="text"
                                       name="start_date"
                                       value="{{date('m/Y', strtotime($work['start_date']))}}"
                                       id="data_inizio_{{$work['id']}}"
                            ...
                            <div class="input-field col s12 m6">
                                <i class="mdi-action-event prefix active"></i>
                                <label class="active"
                                       for="data_fine_{{$work['id']}}">{{ trans('wizard-application.data_fine') }}</label>
                                <input aria-required="true" type="text"
                                       name="end_date"
                                       value="{{date('m/Y', strtotime($work['end_date']))}}"
                                       id="data_fine_{{$work['id']}}"
                                       class="datepicker required" @if($work['is_current']) disabled @endif >

                            </div>
                            <div class="input-field col s6 m6">
                                <input style="margin-left:-10px;margin-top:30px" type="checkbox" name="lavoro_attuale"
                                       id="lavoro_attuale_{{$work['id']}}"
                                       @if($work['is_current']) checked @endif/>
                                <label style="margin-left:-10px;margin-top:30px" class="active"
                                       for="lavoro_attuale_{{$work['id']}}">{{ trans('wizard-application.attualmente_lavoro_qui') }}</label>
                            ...
                            <div class="col s12 m12 right-align">
                                ...
                                <button name="action" type="submit"
                                    ...
                                    <i
                                            class="mdi-maps-rate-review left "></i>{{ trans('profile.aggiorna') }}
                                </button>
                            </div>
